...
- Application Prep has its own Ollama generation settings because it must produce complete JSON rather than a short classification result: `APPLICATION_PREP_THINK=false`, `APPLICATION_PREP_NUM_CTX=65536`, `APPLICATION_PREP_NUM_PREDICT=4096`, and `APPLICATION_PREP_RETRY_NUM_PREDICT` defaults high enough to retry truncation without changing the main analyzer's `OLLAMA_THINK`. The larger Application Prep context window applies only on the Ollama-capable processor, not on the dashboard host or ordinary job-match processing.
...
- The resume variant workflow has Phase 1 through the local PDF export phase in place. It adds per-host resume source settings, encrypted local Google service-account key storage, a basic Settings workflow for Google baseline setup, Google baseline snapshot sync, requirement-specific swap analysis, a click-to-pair matching UI, local PDF export from the synced resume blocks, saved variant history, and `job_resume_variants` records.
- Google resume setup is localhost-only in this release. Run `dashboard.py` on the PC and open it through `http://localhost:<port>` before uploading credentials or syncing the baseline. LAN hosts such as Fruitsalad can still review existing dashboard data, but they should not run Google resume setup/export.
- Resume source mode can be `resume_txt` or `google_doc`. `resume_txt` is review-only for resume variants; applying swaps and exporting a PDF requires a synced Google Doc baseline.
- Resume variant drafts validate a strict swap contract: up to four existing resume bullets can be matched to up to four grounded replacement bullets, a baseline bullet and replacement can each be used only once, and edited replacement wording requires explicit grounding acknowledgment.
- The resume variant API exposes `GET`/`PUT /api/resume-source/settings`, `GET /api/resume-source/google/status`, `PUT /api/resume-source/google/config`, `POST /api/resume-source/google/disconnect`, `POST /api/resume-source/google/select`, `POST /api/resume-source/sync`, `GET /api/application-prep/jobs/<job_pk>/resume-variants/swap-analysis`, `GET`/`POST /api/application-prep/jobs/<job_pk>/resume-variants`, and `GET /api/resume-variants/<variant_id>/download`. The variant `POST` endpoint validates selected swaps, verifies the synced baseline is still current, applies replacements in memory, renders a local PDF, and stores it under `output/resume_variants/`.
- Resume variant draft saves rebuild the current swap analysis server-side and require the browser's analysis hash before saving. If Application Prep or the baseline snapshot changed, reload the board before saving.
- Google service-account keys are encrypted under `data/google_resume/`, which is ignored by git. Set `GOOGLE_RESUME_ENCRYPTION_KEY` or `DASHBOARD_SECRET_KEY` before saving credentials, and keep that value stable across dashboard restarts.
- The synced baseline snapshot stores ordinary Google Docs body-list bullets, section text, document indices, revision metadata, page size/margins, named styles, paragraph settings, inline text runs, list definitions, tables, headers/footers, and a hash. Non-list resume structures are preserved for local PDF rendering but are not modified by bullet swaps.
- Local resume PDF export now resolves Google Docs style inheritance instead of guessing formatting from text. It carries over source page geometry, Arial font faces, font size/weight, bold/italic/underline/color, paragraph alignment/spacing/indents, list glyphs, table column widths, cell padding/backgrounds, and the selected first/default header or footer. For example, a role line using Google Docs `HEADING_2` stays bold even when its individual text run does not contain an explicit `bold` property.
- No database migration or one-time re-sync is required for this formatting upgrade. PDF creation already reads the current Google Doc before export; compatibility hashes accept an unchanged text-only baseline, and the completed variant records the exact style-aware snapshot used to render it. A normal baseline re-sync is still required when the resume content itself changed.
- The Settings page uses the service-account setup path: paste service-account JSON, copy the displayed service-account email, share the baseline Google Doc with that email, paste the baseline document ID, and sync the baseline. A visual document picker is not implemented.
- Service-account baseline sync uses read-only Drive metadata and Docs scopes. Local PDF export intentionally avoids Google Drive `files.copy`, Docs `batchUpdate`, and Drive `files.export` because personal Gmail service accounts can hit Drive ownership/quota failures such as `storageQuotaExceeded`.
- The matching UI appears inside the selected Application Prep job. It requires current Application Prep output and a synced Google Doc baseline, then lets you select one current bullet from **Swap out** and one proposed bullet from **Bring forward** to create a reversible draft pair. The board only suggests swaps when a grounded replacement beats a current bullet against the replacement's specific job requirement; strong current bullets are kept out of the swap-out list.
- Application Prep auto-refresh updates the job deck/status only. It intentionally does not reload the active swap board, because reloading during matching would clear selected bullets or draft pairs.
- `google-api-python-client==2.198.0` is used for Drive and Docs API calls. Snyk lists no direct critical, high, medium, or low issues for this version, and it was published more than 60 days before this phase.
- `google-auth==2.55.1` is used for service-account credentials. Snyk lists no direct issues for this pin.
- `cryptography==50.0.1` is used only for encrypted local service-account key storage. This pin is newer than the normal 60-day preference because Snyk reports high-severity issues in older `cryptography` ranges and identifies the current line as the non-vulnerable fix path.
- `reportlab==4.5.1` is used for local resume PDF rendering. Snyk lists no direct issues for this pin, and it was published more than 60 days before this phase.
...
| `DASHBOARD_SECRET_KEY` | Optional encryption secret fallback for Google resume service-account key storage. Prefer `GOOGLE_RESUME_ENCRYPTION_KEY` when available. |
| `GOOGLE_RESUME_ENCRYPTION_KEY` | Secret used to derive the local encryption key for `data/google_resume/*.enc`. Keep stable or previously saved Google resume credentials cannot be decrypted. |
| `RESUME_VARIANT_HOST_ID` | Optional per-host key for resume source settings, defaulting to the machine hostname. Useful when localhost and a LAN dashboard share the same database. |
...
| `APPLICATION_PREP_NUM_CTX` | Ollama context window used only for Application Prep; default `65536` (never below `OLLAMA_NUM_CTX`). Increase only on the Ollama-capable processor when the installed model supports it. |
| `APPLICATION_PREP_INVENTORY_MAX_TOKENS` | Approximate maximum Responsibilities & Value inventory tokens included in one eligible Application Prep prompt; default `1200`, while the complete Markdown stays stored in the database. |
